...
flowchart TD
    subgraph 顶层 优先级最高
        A["① CLI 命令行参数（最高优先级）\n--model、--config、--profile"]:::cli
            noteA["单次运行覆盖"]
    end


   subgraph *
    B["② 项目级 .codex/config.toml\n仓库内 .codex/ 目录（需项目被信任）"]:::project
    noteB["项目级强制"]
   end
   
   subgraph *
    C["③ Profile 命名配置（--profile）\n~/.codex/&lt;name&gt;.config.toml"]:::profile
    noteC["场景切换"]
   end
        
    subgraph ***
    D["④ 用户级 ~/.codex/config.toml\nCODEX_HOME 目录下的个人默认配置"]:::user
        noteD["个人默认"]
    end

    subgraph 底层 优先级最低
        E["⑤ 系统级 / 团队共享配置\nTeam Config 共享规则与系统级文件"]:::system
            noteE["团队基线"]
    end
