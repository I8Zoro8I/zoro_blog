...
词嵌入通过学习一个**嵌入矩阵**来解决这个问题：
$$
E \in \mathbb{R}^{V \times D}
$$
其中 V*V* 是词表大小，D*D* 是嵌入维度。每个词 ID 对应嵌入矩阵中的一行，通过查表操作获取其向量表示：
$$
...
class TextClassificationModel(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = self.fc(dropped)  # 修正了之前的拼写错误
        return logits
...
# ── 主程序 ────────────────────────────────────────
# 1. 模拟数据：生成一些随机索引作为文本输入
dummy_data = torch.randint(2, VOCAB_SIZE, (20, MAX_LEN)).tolist()
print(dummy_data)
dummy_labels = [0, 1] * 10

# 2. 初始化
dataset = TextDataset(dummy_data, dummy_labels, MAX_LEN)
loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True)
model = TextClassificationModel(VOCAB_SIZE, EMBED_DIM, HIDDEN_SIZE, NUM_LAYERS, NUM_CLASSES, DROPOUT).to(device)
