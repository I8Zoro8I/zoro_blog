...
| 数据量             | 与源任务相似度 | 推荐策略                       |
| :----------------- | :------------- | :----------------------------- |
| 少（< 1000）       | 高             | 只替换最后分类头，冻结全部主干 |
| 少（< 1000）       | 低             | 微调较浅的层，冻结深层         |
| 中等（1000~10000） | 高             | 微调全部网络，用小学习率       |
| 中等（1000~10000） | 低             | 微调较深的层，冻结浅层         |
| 多（> 10000）      | 任意           | 全部微调，或考虑从头训练       |
...
| 问题              | 推荐做法                                        |
| :---------------- | :---------------------------------------------- |
| 数据量 < 500      | 冻结所有层，只训练分类头                        |
| 数据量 500~2000   | 冻结前 2/3，微调后 1/3                          |
| 数据量 2000~10000 | 全量微调，主干用 1e-5，头部用 1e-3              |
