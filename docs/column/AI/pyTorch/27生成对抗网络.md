...
$$
\min_{G} \max_{D} \mathbb{E}_{x \sim p_{\text{data}}(x)} \left[ \log D(x) \right] + \mathbb{E}_{z \sim p_z(z)} \left[ \log\left(1 - D(G(z))\right) \right]
$$
其中：
...
固定生成器，提升判别器的分辨能力：
$$
\max_{D} \mathbb{E}_{x \sim p_{\text{data}}} \left[ \log D(x) \right] + \mathbb{E}_{z \sim p_z} \left[ \log\left(1 - D(G(z))\right) \right]
$$
**第二阶段：训练生成器**

固定判别器，提升生成器的欺骗能力：
$$
\min_{G} \mathbb{E}_{z \sim p_z} \left[ \log\left(1 - D(G(z))\right) \right]
$$
