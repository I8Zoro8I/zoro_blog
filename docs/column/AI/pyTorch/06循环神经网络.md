...
- 输入（xt*x**t*）：在时间步 t*t* 的输入向量。
- 隐藏状态（ht*h**t*：在时间步 t*t* 的隐藏状态向量，用于存储之前时间步的信息。
- 输出（yt*y**t*）：在时间步 t*t* 的输出向量（可选，取决于具体任务）。
...
![img](https://www.runoob.com/wp-content/uploads/2024/12/1_dznTsiaHCvRc70fxWWEcgw.png)
...
![img](https://www.runoob.com/wp-content/uploads/2024/12/rnn_loss_runoob.png)
