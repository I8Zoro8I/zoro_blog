...
$$
\min_{\theta, \phi} \frac{1}{n} \sum_{i=1}^{n} \left\| x_i - D_\phi(E_\theta(x_i)) \right\|^2
$$
其中 *θ* 是编码器参数，*ϕ* 是解码器参数。
...
- 编码器输出均值 μ*μ* 和标准差 σ*σ*
- 从正态分布 N(μ,σ)N(*μ*,*σ*) 中采样得到潜在向量 z*z*
- 解码器从 z*z* 重构数据

为了实现可微的采样过程，使用了**重参数化技巧（Reparameterization Trick）**：
$$
