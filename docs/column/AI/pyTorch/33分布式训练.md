...
1. **模型复制**：将模型复制到每个 GPU 上
2. **数据分发**：将 batch 数据均匀分配到各个 GPU
3. **并行计算**：每个 GPU 独立计算前向传播
4. **梯度聚合**：将所有 GPU 的梯度汇总到主 GPU
5. **参数更新**：主 GPU 更新参数后同步到其他 GPU
...
1. 每个 GPU 将梯度分为 n 份（n 为 GPU 数量）
2. 每个 GPU 与相邻 GPU 交换一份梯度
3. 重复 n 次后，每个 GPU 拥有完整的梯度
