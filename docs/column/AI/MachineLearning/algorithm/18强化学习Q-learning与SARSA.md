...
flowchart TD
    A[重置环境开始新一轮训练]
    B[观察当前状态s_prime作为新的s]
    C{根据epsilon贪婪策略选择动作a}
    D[执行动作a与环境交互获得即时奖励r进入新状态s_prime]
    E[估计下一状态最优价值max_a_primeQ_s_prime_a_prime]
    F[计算TD目标值]
    G{s_prime是否为终止状态如到达终点或步数耗尽}
    H{是否完成所有训练轮次}
    I[输出最优策略每个状态选maxQ值动作]
