...
- **环境**： 是智能体所处的外部世界，智能体与之互动。对于小狗来说，环境就是你、零食、地板等一切外部事物。环境会接收智能体的动作，并给出新的状态和奖励。
  ...
  ```mermaid
  graph TD
      Env[Environment 环境] -->|观察/感知| Interp[Interpreter 解释器]
      Interp -->|State 状态| Ag[Agent 智能体]
      Interp -->|Reward 奖励| Ag
      Ag -->|Action 动作| Env
  ```
...
graph TD
    ...
    Policy[根据策略选择动作 At]
    Receive[环境接收动作 At]
    Update[环境转移到新状态 St+1 <br/>并给出奖励 Rt+1]
...
```mermaid
graph TD
    S_t[当前状态 St] -->|智能体观察并动作| A_t[执行动作 At]
    A_t -->|环境响应| R_t1[获得奖励 Rt+1]
    A_t -->|环境状态转移| S_t1[进入新状态 St+1]
    
    R_t1 -->|闭环迭代| S_t1
    S_t1 -->|作为下一轮的输入| S_t_next[新一轮当前状态 St]
```
