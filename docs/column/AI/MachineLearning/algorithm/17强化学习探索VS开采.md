...
graph TD
    ...
    Init[初始化：为每个动作维护奖励参数的先验分布]

    Step{每个时间步}

    Sample[从每个动作的当前后验分布中采样参数值θ_a]

    Select[选择采样值θ_a最大的动作执行]

    Reward[执行动作观察环境返回的真实奖励r]

    Update[根据奖励r更新所选动作的后验分布r=1→α=α+1<br/>r=0→β=β+1]

    EndCheck{是否达到终止条件?如迭代次数/收敛}

    Result[输出最优策略选择后验期望最大的动作]
