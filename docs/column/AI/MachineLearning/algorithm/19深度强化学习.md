...
![img](https://www.runoob.com/wp-content/uploads/2025/12/ml-deep-reinforcement-learningx45.png)
...
![img](https://www.runoob.com/wp-content/uploads/2025/12/6f326e47-82f6-4333-810f-dd43c313753.png)
...
4. 从回放池中随机采样一批经验。
5. 对于每个样本，计算目标 Q 值：$y = r + \gamma \max_{a'} Q_{\text{target}}(s', a')$
6. `y = r + γ * max_a' Q_target(s', a')`。其中 `γ` 是折扣因子，用于权衡即时奖励和未来奖励。
7. 以 `(y - Q(s, a))^2` 作为损失，通过梯度下降更新在线网络 `Q` 的参数。
8. 每隔一定步数，将在线网络的参数复制给目标网络。
9. 重复步骤 2-7。
...
$$
 `θ ← θ + α * ∇_θ log π(a|s; θ) * G_t` 其中 `G_t` 是从当前时刻到回合结束的累积奖励，`α` 是学习率。
