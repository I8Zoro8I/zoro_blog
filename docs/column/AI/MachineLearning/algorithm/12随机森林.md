...
graph TD
    %% 节点定义
    Start([开始])
    SetN[设置森林中树的数量<br/>n_estimators]
    LoopStart{对于每一棵树 i = 1 to n}
    
    Bootstrap[Bootstrap 采样<br/>生成第 i 个训练子集]
    SelectFeat[随机选取特征子集]
    TrainTree[用子集和特征子集<br/>训练一棵决策树]
    ...
    Classify[统计所有树的预测类别<br/>取票数最多的类别]
    OutClass[输出最终类别]
    
    Regress[计算所有树预测值的平均值]
    OutRegress[输出最终数值]
    ...
    SetN --> LoopStart
    LoopStart --> Bootstrap
    Bootstrap --> SelectFeat
    SelectFeat --> TrainTree
    TrainTree --> CheckTree
    
    CheckTree -- 否 --> LoopStart
    CheckTree -- 是 --> NewSample
    
    NewSample --> Predict
    Predict --> TaskType
    
    TaskType -- 分类 --> Classify
    Classify --> OutClass
...
![img](https://www.runoob.com/wp-content/uploads/2025/12/2faf4677-a00e-4a7a-a798-367d35b14742.png)
