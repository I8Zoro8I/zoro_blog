...
```mermaid
graph LR
    W1(Weak learner 1) --> S[Strong learner]
    W2(Weak learner 2) --> S
    W3(Weak learner 3) --> S
    W4(Weak learner 4) --> S
    W5(Weak learner 5) --> S

    style S fill:#3498db,stroke:#2980b9,stroke-width:2px;
```
...
graph LR
    ...
    BS1[Bootstrapped samples 1]
    BS2[Bootstrapped samples 2]
    BS3[Bootstrapped samples 3]
...
graph TD
    %% 阶段 1
    TD1[Training data] --> M1[Model #1]
    M1 --> IP1[Incorrect predictions]
    M1 --> CP1[Correct predictions]

    %% 阶段 2
    IP1 -->|加重错误样本权重| WD2[Weighted data]
    WD2 --> M2[Model #2]
    M2 --> IP2[Incorrect predictions]
    M2 --> CP2[Correct predictions]

    %% 阶段 n
    IP2 -->|继续调整权重 ...| WDn[Weighted data]
    WDn --> Mn[Model #n]
    Mn --> IPn[Incorrect predictions]
    Mn --> CPn[Correct predictions]

    %% 样式调整
    style IP1 fill:#fdebd0,stroke:#e67e22
    style IP2 fill:#fdebd0,stroke:#e67e22
    style IPn fill:#fdebd0,stroke:#e67e22
    style CP1 fill:#d5f5e3,stroke:#27ae60
    style CP2 fill:#d5f5e3,stroke:#27ae60
    style CPn fill:#d5f5e3,stroke:#27ae60
```
...
graph LR
    ...
    %% 样式调整
    style D fill:#badcff,stroke:#3498db
    style ML fill:#fff,stroke:#333
    style FP font-weight:bold
```
