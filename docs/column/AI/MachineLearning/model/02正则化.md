...
flowchart LR
    A[模型总误差] --> B[偏差 Bias]
    A --> C[方差 Variance]
    A --> D[不可避免的噪声 Irreducible Error]

    %% 偏差分支
    B --> B1[定义：模型预测值的期望与真实值之间的差异]
    B --> B2[高偏差表现：欠拟合，模型过于简单，无法捕捉数据规律]
    B --> B3[比喻：一直瞄准靶心左边射击的弓箭手]

    %% 方差分支
    C --> C1[定义：模型预测值自身的离散程度/波动范围]
    C --> C2[高方差表现：过拟合，模型过于复杂，对训练数据敏感]
    C --> C3[比喻：瞄准点很分散的弓箭手，每次落点都不同]

    %% 噪声分支
    D --> D3[比喻：瞄准点很分散的弓箭手，每次落点都不同]
```
