...
本章是全书里第一篇把 **RAG 与知识库** 落到产品和平台上的章节。前面的 [RAG、微调、续训与智能体选型](https://i8zoro8i.github.io/zoro_blog/column/AIAgent/01-大模型基础能力构建/03-RAG、微调、续训与智能体选型) 更偏“为什么选 RAG、什么时候该选 RAG”；本章更偏“如果已经决定用 RAG，该怎样把知识库搭起来”；后面的 [RAG检索增强生成](https://i8zoro8i.github.io/zoro_blog/column/AIAgent/03-大模型核心开放框架/11-RAG检索增强生成) 会继续从 LangChain 代码侧，把文档加载、切块、Embedding、向量库、检索与生成串成完整工程链路。
...
**第一，时间维度。** 随着模型规模增大，训练成本和周期也更高，因此大模型不可能天天重新训练。于是“最新信息、刚变更的信息、实时更新的信息”天然不是它的强项。

**第二，数据来源维度。** 大模型主要学到的是公开语料，而企业内部资料、个人笔记、项目规范、客户材料、课程总结，往往根本不在训练范围内。你直接问模型，它要么不知道，要么半猜半答。
...
![RAG 通过外部知识提示模型答题并提升正确率的示意图](https://didilili.github.io/ai-agents-from-zero/images/2/2-1-2-1.png)

**举例 2：**

![RAG 结合外部资料回答领域问题的示意图](https://didilili.github.io/ai-agents-from-zero/images/2/2-1-2-2.png)
...
flowchart LR
    ...
    %% 阶段一：知识库构建（离线）
    %% =========================
    subgraph S1["阶段一：知识库构建（离线）"]
        direction LR

        A["原始资料<br/>PDF / Word / Markdown / 网页"]
        B["文档解析与清洗"]
        C["文本切分<br/>Chunk"]
        D["Embedding<br/>文本转向量"]
        E["向量库 / 知识库<br/>保存文本块、向量、元数据"]
    ...
    %% 阶段二：检索与生成（在线）
    %% =========================
    subgraph S2["阶段二：检索与生成（在线）"]
        direction LR

        F["用户问题"]
        G["问题向量化"]
        H["检索相关文本块"]
        I["Prompt 增强<br/>问题 + 检索结果"]
        J["大模型生成"]
        K["带依据的回答"]
    ...
    %% 样式
    %% =========================
    classDef node fill:#252525,color:#fff,stroke:#888,stroke-width:1px;
    classDef group fill:#ffffb3,stroke:#ddd,stroke-width:1px,color:#aaa;

    class A,B,C,D,E,F,G,H,I,J,K node;
```

![LangChain 与 ChatGLM 构建知识库及问答流程的整体示意图](https://didilili.github.io/ai-agents-from-zero/images/2/2-1-3-1.png)
...
![RAG 从知识库构建到检索增强生成的完整架构图](https://didilili.github.io/ai-agents-from-zero/images/2/2-1-3-2.png)
...
如果你后面继续看 [RAG检索增强生成](https://i8zoro8i.github.io/zoro_blog/column/AIAgent/03-大模型核心开放框架/11-RAG检索增强生成)，会发现这四个词会分别落到更具体的代码对象上，例如：
...
![RAG 中最容易影响效果的关键步骤示意图](https://didilili.github.io/ai-agents-from-zero/images/2/2-1-3-3.png)
...
flowchart TD
    A[RAG效果不好] --> B{完全找不到资料?}
    B -- 是 --> C[查入库链路<br/>文档是否上传、解析是否成功、Embedding是否一致]
    B -- 否 --> D{召回内容答非所问?}
    D -- 是 --> E[查检索策略<br/>关键词检索、向量检索、Top K、阈值、Rerank]
    D -- 否 --> F{资料找到了但模型乱答?}
    F -- 是 --> G[查Prompt约束<br/>仅基于资料回答、允许不知道、要求引用来源]
    F -- 否 --> H{答案不完整或漏重点?}
    H -- 是 --> I[查上下文组织<br/>Chunk大小、Overlap、Top K、上下文窗口]
    H -- 否 --> J{无法追溯来源?}
    J -- 是 --> K[查元数据与引用<br/>文件名、页码、标题、段落ID]
    J -- 否 --> L[整理一组测试问题<br/>记录召回片段、引用和最终答案]
```
...
**小型企业主或创业者：** 经常要查阅和分享制度文档、产品说明、客户反馈、市场分析。把这些资料整理成知识库后，问答和复用效率会明显提高。

**职场打工人或自由职业者：** 无论是写作、设计、开发，还是视频制作，你都会积累大量素材、需求、规范和项目资料。知识库的价值不是“存起来”，而是“下次还能快速找到并继续用”。

**教育工作者或学生：** 老师可以整理课程资料、讲义、案例、答疑文档；学生可以把课堂笔记、论文资料、考试重点、知识点总结做成自己的复习型知识库。

**生活中的普通人：** 旅行计划、阅读笔记、兴趣资料、学习记录、求职资料，全部都可以做成个人知识库。知识库不是企业专属工具，它同样适合个人成长与日常信息管理。
...
![Cherry Studio、AnythingLLM、Dify、FastGPT 与 RAGFlow 等知识库平台对比图](https://didilili.github.io/ai-agents-from-zero/images/2/2-2-2-1.png)
...
![Cherry Studio 一问多答功能界面示意图](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-1-1.png)

**助手市场**：内置千余个行业专用助手，涵盖翻译、编程、写作等场景，同时支持自定义助手。

![Cherry Studio 助手市场界面示意图](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-1-2.png)

**服务商模型聚合**：支持 OpenAI、Gemini、Anthropic、Azure 等规范的三方服务商接入，兼容性较强。

![Cherry Studio 服务商模型聚合配置界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-1-3.png)
...
![在硅基流动平台创建 API 密钥的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-2-1.png)

**步骤 4：复制 API 密钥**

**步骤 5：配置 API 密钥**

![在 Cherry Studio 中配置 API 密钥的界面一](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-2-2.png)

**步骤 6：选择大语言模型**

![在 Cherry Studio 中选择大语言模型的界面一](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-2-3.png)

![在 Cherry Studio 中选择大语言模型的界面二](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-2-4.png)
...
![确认嵌入模型名称的界面示意图](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-1.png)

回到 Cherry Studio 添加：

![在 Cherry Studio 中添加嵌入模型的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-2.png)

这一步对应的是 RAG 里的“Embedding”环节。后续上传文档建库、用户提问检索，都会依赖这一模型。

**步骤 2：创建知识库**

![在 Cherry Studio 中创建知识库的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-3.png)

提供知识库内容：

![在 Cherry Studio 中向知识库导入文件、网页或文本内容的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-4.png)
...
![Cherry Studio 知识库直接检索结果界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-5.png)
...
![Cherry Studio 基于知识库生成回答的界面一](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-6.png)

![Cherry Studio 基于知识库生成回答的界面二](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-7.png)
...
![Doc2X 文档解析工具官网界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-3-8.png)
...
![个人知识库从导入资料到检索生成的整体流程图](https://didilili.github.io/ai-agents-from-zero/images/2/2-3-4-1.jpg)
...
![ima 登录界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-2-1.png)

**步骤 2：新建知识库**

![在 ima 中新建知识库的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-2-2.png)

**步骤 3：导入本地文件**

![在 ima 中导入本地文件的界面三](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-2-3.png)

**步骤 4：基于知识库「生成」**

![ima 基于知识库生成回答的界面一](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-2-4.png)

![ima 基于知识库生成回答的界面二](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-2-5.png)
...
![在微信公众号中查找可加入知识库的文章界面一](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-3-1.png)

步骤 3：文章导入完成后，即可生成知识库，并基于大模型进行检索与问答。

![ima 基于网页文章构建知识库后的界面一](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-3-2.png)
...
![ima 查看第三方知识库的界面一](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-4-1.png)

![ima 查看第三方知识库的界面二](https://didilili.github.io/ai-agents-from-zero/images/2/2-4-4-2.png)
...
![在 Dify 中创建知识库的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-5-3-1.png)

**步骤 2：上传知识库文件**

这里准备的是一份《刑法》的 TXT 格式文本，按自然段划分了每一条法条。

![在 Dify 中上传知识库文件的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-5-3-2.png)
...
![Dify 知识库分段设置界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-5-3-3.png)

**步骤 4：选择索引方式**

这里自动选择高质量。高质量的准确性更高，但 token 消耗也会增加；如果用的是本地部署模型，成本敏感度会低一些。

![Dify 知识库索引方式选择界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-5-3-4.png)
...
![Dify 知识库检索设置界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-5-3-5.png)
...
![Dify 保存并处理知识库的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-5-3-6.png)

这一节最值得你带走的工程结论是：

> **RAG 的效果，不是“上传完文件”那一刻决定的，而是由文档质量、切块策略、Embedding 质量、检索策略和重排策略共同决定的。**

如果你想继续把这些平台里的配置项和代码实现一一对上，可以把这一节和 [19-RAG检索增强生成](https://didilili.github.io/ai-agents-from-zero/#/19-RAG检索增强生成) 的“文档加载器、文本分割器、检索器”几节对照着看，理解会更快。
...
```text
你是一个法律小助手，请只根据知识库中的信息，简要回答用户提问的案件触犯了哪些法律复制错误已复制
```
...
![Dify 中基于知识库进行测试问答的界面](https://didilili.github.io/ai-agents-from-zero/images/2/2-5-4-1.png)
...
- 如果你想继续往平台化应用和工作流方向走，建议接着看 [基于Coze&Dify平台的智能体开发](https://i8zoro8i.github.io/zoro_blog/column/AIAgent/02-企业低代码平台开发与项目实战/01-基于Coze&Dify平台的智能体开发) 和 [Python调用Dify平台工作流](https://i8zoro8i.github.io/zoro_blog/column/AIAgent/02-企业低代码平台开发与项目实战/智能体的调用与部署/02-Python调用Dify工作流)。
- 如果你想从代码角度真正把 RAG 跑起来，建议先看 [向量数据库与Embedding实战](https://i8zoro8i.github.io/zoro_blog/column/AIAgent/03-大模型核心开放框架/10-向量数据库与Embedding实战)，再继续看 [RAG检索增强生成](https://i8zoro8i.github.io/zoro_blog/column/AIAgent/03-大模型核心开放框架/11-RAG检索增强生成)。