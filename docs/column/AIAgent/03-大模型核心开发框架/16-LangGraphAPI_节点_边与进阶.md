...
```mermaid
flowchart LR
    START([START]) --> node_a["路由节点<br/>node_a"]
    node_a --> route{"route_fn(state)"}
    route -- condition_1 --> node_1["node_1"]
    route -- condition_2 --> node_2["node_2"]
    route -- condition_3 --> node_3["node_3"]
    node_1 -.-> END([END])
    node_2 --> END
    node_3 -.-> END
```
...
```mermaid
flowchart LR
    START([START]) --> TASK[生成任务列表]
    TASK --> SEND[条件边返回<br/>Send 列表]

    SEND -->|参数 1| W1[worker]
    SEND -->|参数 2| W2[worker]
    SEND -->|参数 3| W3[worker]

    W1 --> REDUCE[Reducer 汇总结果]
    W2 --> REDUCE
    W3 --> REDUCE
...
flowchart LR
    ...
    COMMAND -->|goto| GOTO["goto<br/>指定下一跳"]
    COMMAND -->|update| UPDATE["update<br/>写回局部 State"]

    GOTO --> TARGET["目标节点或 END"]

    UPDATE --> REDUCE["按 Reducer 合并"]
    REDUCE -.->|合并后的状态| TARGET
```
