...
- 会运行并理解本章全部案例：**输入类型、同步与异步调用、文本模板、对话模板、消息占位符、从 JSON / YAML 加载提示词**，为后续 输出解析器、LCEL 与链式调用、记忆与对话历史 打基础。
...
本章对应 第 3 章 Model I/O 里的 **输入格式化（Format）** 与 **模型调用（Predict）** 两部分。模型并不是“凭空思考”，它始终是在读取我们提供的输入后再生成输出；而 Prompt、Message、Template，正是组织这份输入的核心工具。

如果你已经学过  提示词工程基础，那么本章可以直接理解为它的代码实现版：前一章讲“怎么把角色、任务、上下文、输入、输出和约束说清楚”，这一章讲“这些内容进入 LangChain 后，应该以什么输入形态存在、如何复用、如何插入多轮历史、如何外置到文件”。
...
![DeepSeek 官方提示词示例库入口](https://didilili.github.io/ai-agents-from-zero/images/13/13-1-1-1.jpeg)
...
这一章不是零散的 API 介绍，而是在回答一个问题：**开发 LLM 应用时，怎样把  提示词工程基础 里学到的写法和原则，组织成一套可复用、可维护、可扩展的结构？**

------

## 2、调用大模型的入参类型

初学者容易误以为：调用聊天模型时，输入永远只能是“一段字符串”。实际上，LangChain 聊天模型为了适配真实对话场景，通常支持多种输入形态。这些写法表面不同，核心都一样：**把 基础能力构建 里讲过的角色、任务、上下文、输入和约束，以合适的消息结构交给模型。**

### 2.1 入参形态总览

同一次 `invoke`，左侧可以是多种类型的输入，中间由聊天模型处理，右侧典型返回值是 `AIMessage`。这也是为什么你在 [第 3 章]会经常看到“正文一般通过 `.content` 读取”。

![聊天模型 invoke：常见入参类型与 AIMessage 输出](https://didilili.github.io/ai-agents-from-zero/images/13/13-2-1-1.png)
...
这正对应了 基础能力构建-提示词工程基础 里强调的那条原则：**稳定约束和动态输入要拆开写。**在 LangChain 里，这条原则最直接的落地方式就是把稳定规则放进 `SystemMessage`，把当前任务放进 `HumanMessage`。
...
- **本地开源模型（Ollama 等）**：实例化与端点配置见 第 4 章 Ollama 本地部署与调用，本章的 `invoke` / `stream` / `batch` 用法同样适用。

```py
"""
【案例】模型调用：同步 invoke（单次调用，一次性返回）

对应教程章节：第 5 章 - 提示词与消息模板 → 4、调用大模型的调用方式
...
对应教程章节：第 5 章 - 提示词与消息模板 → 6、文本提示词模板（PromptTemplate）
...
- `format(...)` 之后得到的是字符串，这一点与第 5 章正文的主线保持一致。
"""
...
对应教程章节：第 5 章 - 提示词与消息模板 → 7、对话提示词模板（ChatPromptTemplate）
...
![MessagesPlaceholder 将历史消息动态插入 ChatPromptTemplate：模板先留占位，运行时再填入多轮消息](https://didilili.github.io/ai-agents-from-zero/images/13/13-7-5-1.svg)
...
对应教程章节：第 5 章 - 提示词与消息模板 → 7.5、MessagesPlaceholder：消息占位符
...
运行这类案例时，先确认**当前工作目录**，否则相对路径可能找不到文件。这一点和 第 10 章 HelloWorld 中的 `.env` 读取问题本质很像，都是“脚本运行位置”和“相对路径”的关系。
...
对应教程章节：第 5 章 - 提示词与消息模板 → 8、从文件加载提示词（JSON / YAML）
