...
**学习建议：** 这一章先当成 LangChain 的地图，不急着记 API。读完最好能说清三件事：LangChain 帮你组织哪些能力、它不负责哪些底层能力、1.x 写法和旧资料里的 0.x 写法为什么会不一样。看完后直接进入 下一章 跑一次最小调用，抽象概念会落得更稳。
...
![LangChain 主仓库（langchain-ai/langchain）GitHub Star 历史增长曲线，2023 初至 2025 初由近零增长至约 10 万+（star-history.com 风格图）](https://didilili.github.io/ai-agents-from-zero/images/9/9-1-1-1.png)
...
![大语言模型单独使用时的典型局限：知识截止、无实时联网、难接私有数据与数据库、输出稳定性与工具调用等（示意图）](https://didilili.github.io/ai-agents-from-zero/images/9/9-1-2-1.png)
...
![JDBC 与 LangChain 的类比：左侧为 Java 经 JDBC 对接多种数据库；右侧为 AI 应用经 LangChain 对接多种模型 API（直连单一 API「可行但不建议」的示意）](https://didilili.github.io/ai-agents-from-zero/images/9/9-1-3-1.jpg)
...
![LLM 应用四层架构：UI 交互层、服务/链层、模型层、存储层及数据流方向；LangChain 主要处于服务/链层](https://didilili.github.io/ai-agents-from-zero/images/9/9-2-2-1.png)
...
![招聘场景中简历片段示意](https://didilili.github.io/ai-agents-from-zero/images/9/9-2-4-1.png)
...
![LangChain V0.1 时期架构示意：以链（Chain）与组件组合为主线的经典教学图](https://didilili.github.io/ai-agents-from-zero/images/9/9-3-1-1.gif)

**第二阶段：0.2 / 0.3 阶段，开始重视生态拆分和工程边界。**
这一时期官方逐渐把 LangChain、集成包、部署与调试能力拆开，形成更清晰的产品层次。你也会在很多 2024 年左右的教程里看到这套结构图。

![LangChain V0.2/V0.3 生态：架构、组件、部署三层示意](https://didilili.github.io/ai-agents-from-zero/images/9/9-3-1-2.gif)
...
![LangChain 1.x 轻核心示意：langchain-core 为底座，主包精简、集成拆至各 provider 包并与 LangGraph 等协作](https://didilili.github.io/ai-agents-from-zero/images/9/9-3-1-3.gif)
...
![LangChain 0.x 与 1.x 核心差异：旧写法多步组装，新主线强调统一入口、create_agent 与 LangGraph Runtime](https://didilili.github.io/ai-agents-from-zero/images/9/9-3-3-1.jpeg)
...
这里有一个容易混淆的点： **并不是说 1.x 以后“链”就没用了，而是说很多旧式高层 Chain 类不再是官方推荐主线。**课程后面的 第 7 章 LCEL 与链式调用 仍然是主线内容，因为“把多个步骤组合起来”的思想从来没有过时，只是实现方式从“某个现成 Chain 类”逐渐转向 **Runnable / LCEL / Graph**。
...
![LangChain 完整知识体系：从模型接入、Prompt、Parser、LCEL 到 Retrieval、Tools、MCP、Agent 的学习路线](https://didilili.github.io/ai-agents-from-zero/images/9/9-4-1-1.png)
...
![LangChain Model I/O：Format（输入格式化）→ Predict（调用模型）→ Parse（解析输出）三阶段关系示意](https://didilili.github.io/ai-agents-from-zero/images/9/9-4-2-1.png)
...
-  HelloWorld：先学会把模型调起来
- 第 3 章 Model I/O：理解模型接入与参数
- 第 5 章 提示词与消息模板：理解 Prompt、Message
- 第 6 章 输出解析器：理解结构化输出
...
- 第 8 章 记忆与对话历史（含 Redis 基础）

### 4.5 Retrieval（检索）：检索与 RAG

**Retrieval** 模块解决的是“模型不知道的知识从哪里来”的问题。它是 RAG 的核心组成部分，负责从外部知识源中检索和当前问题相关的信息，再把检索结果交给模型辅助生成答案。 ![LangChain Retrieval：外部知识接入与检索增强生成（RAG）在应用中的位置示意](https://didilili.github.io/ai-agents-from-zero/images/9/9-4-5-1.png)
...
![LangChain Agents：在工具（Tools）之上由模型进行规划、选工具与多步执行的典型关系示意](https://didilili.github.io/ai-agents-from-zero/images/9/9-4-6-1.png)
...
![LangChain 六大核心模块关系总览：Models、Memory、Retrieval、Chains、Agents、Callbacks（教学向总图，与官方 1.x 文档栏目划分是粗粒度对应关系）](https://didilili.github.io/ai-agents-from-zero/images/9/9-4-8-1.jpeg)
...
![LangChain 六大模块展开小结：Agent、Chain、Memory、Data Connection、Model I/O、Callbacks 及其子组件在一页上的关系](https://didilili.github.io/ai-agents-from-zero/images/9/9-4-8-2.jpg)
...
**建议下一步：** 直接进入  LangChain 快速上手与 HelloWorld，先把 **API Key、模型名、Base URL、HelloWorld 调用、多模型共存、企业级封装与流式输出** 跑通。等你真正把第一个 LangChain 调用写出来，再回头看这一章的“定位、架构、模块”，理解会明显更扎实。
