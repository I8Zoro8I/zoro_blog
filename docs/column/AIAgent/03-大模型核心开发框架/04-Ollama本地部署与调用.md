...
- 学会使用 **LangChain** 里的 `ChatOllama` 调用本机 Ollama 服务，理解它与 第 3 章 Model I/O 与模型接入 中云端模型调用的异同。
...
![Ollama 官方标志：羊驼线稿风格图标（品牌识别，常见于安装包与文档）](https://didilili.github.io/ai-agents-from-zero/images/12/12-1-1-1.png)
...
![Windows 安装向导中自定义 Ollama 程序安装路径的操作步骤（动图）](https://didilili.github.io/ai-agents-from-zero/images/12/12-2-4-1.gif)
...
![Windows「环境变量」对话框中新建用户变量 OLLAMA_MODELS 并指向自定义 models 目录](https://didilili.github.io/ai-agents-from-zero/images/12/12-2-5-1.jpeg)
...
![将已有 blobs、manifests 等模型目录内容复制到 OLLAMA_MODELS 指定路径以复用下载](https://didilili.github.io/ai-agents-from-zero/images/12/12-2-6-1.jpeg)
...
![Ollama 桌面客户端菜单中打开 Settings（设置）的入口位置](https://didilili.github.io/ai-agents-from-zero/images/12/12-2-7-1.png)

![在 Settings 中将 Model location 改为课程资料或自定义的 models 文件夹](https://didilili.github.io/ai-agents-from-zero/images/12/12-2-7-2.png)
...
![本地 LangChain 应用连接 Ollama 服务：代码发送请求，本地服务加载模型文件并返回 AI 回复](https://didilili.github.io/ai-agents-from-zero/images/12/12-5-1-1.png)
...
如果你希望和 第 5 章 提示词与消息模板 保持一致，也可以传消息列表：
...
对应教程章节：第 4 章 - Ollama 本地部署与调用 → 5、LangChain 整合 Ollama 调用本地大模型

知识点速览：
- 本案例对应第 4 章里“把本地 Ollama 服务接进 LangChain”的最小落地示例。
...
**建议下一步：** 先亲手完成这条最小链路：安装 Ollama → `ollama run qwen:4b` 跑通一次本地模型 → 用 LangChain_Ollama.py 在 Python 里调通本机模型。跑通之后，再进入 第 5 章 提示词与消息模板、第 6 章 输出解析器、第 7 章 LCEL 与链式调用，把本地模型也串进完整的 LangChain 工作流里。
