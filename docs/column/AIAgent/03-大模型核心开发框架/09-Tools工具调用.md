...
- 跑通并理解本章全部案例：**基础加法工具、Pydantic 参数 schema、天气查询工具、天气助手完整链路**，为后续 记忆与对话历史、Agent 智能体、MCP 模型上下文协议 打基础。
...
![未开启联网或工具时，对话产品无法给出实时天气，只能提示用户开启能力（示意图）](https://didilili.github.io/ai-agents-from-zero/images/17/17-1-2-1.jpeg)
...
所以本章的重点，是先把 **“工具本身”** 和 **“单轮/手动工具调用链路”** 学明白。到了 第 5 章 Agent 智能体，你再去理解“让模型自己循环地、多步地调用工具”，就会轻松很多。
...
![工具调用泳道图：用户、程序、工具、大模型四栏协作（用户提问 → 程序带工具定义调模型 → 模型判断是否调用工具 → 程序执行工具 → 结果再交模型生成回复）](https://didilili.github.io/ai-agents-from-zero/images/17/17-2-1-1.jpeg)
...
如果你看过 第 3 章 和 第 5 章，这里会看到一条连续的消息主线：

- 第 3 章讲的是模型返回 `AIMessage`
- 第 5 章讲的是消息流里有 `ToolMessage`
- 第 9 章就是把这两件事真正接起来

也就是说，**Tool Calling 不是脱离消息机制另起炉灶，它本质上仍然发生在消息流之中。**

![bind_tools 与 Agent 自动执行工具的边界：绑定工具只提供 schema，Agent 才负责循环执行工具并写回观察结果](https://didilili.github.io/ai-agents-from-zero/images/17/17-2-1-2.svg)
...
![LangChain 文档摘录：使用 @tool 装饰器创建工具，默认以函数 docstring 作为工具描述（Basic tool definition）](https://didilili.github.io/ai-agents-from-zero/images/17/17-3-1-1.jpeg)
...
对应教程章节：第 9 章 - Tools 工具调用 → 3、自定义 Tool：先从最简单的工具开始 → 3.1 使用 @tool 装饰器 / 3.2 基础案例：加法工具
...
![@tool 与函数名、类型注解、docstring 在工具定义中的角色示意（装饰器注册、函数名为工具 ID、类型帮助生成参数、文档字符串为「使用说明」）](https://didilili.github.io/ai-agents-from-zero/images/17/17-3-3-1.jpeg)
...
对应教程章节：第 9 章 - Tools 工具调用 → 4、参数 schema：为什么要配合 Pydantic → 4.2 Pydantic 定义 / 4.3 入门案例
...
对应教程章节：第 9 章 - Tools 工具调用 → 4、参数 schema：为什么要配合 Pydantic → 4.4 加法工具的 Pydantic 版
...
- **OpenWeather API Key**：在 OpenWeather API keys 页面 免费申请，将密钥写入**项目根目录** `.env`（例如 `OPENWEATHER_API_KEY=你的密钥`），并保证运行脚本时能加载到该变量。天气 HTTP API 总览见 OpenWeatherMap 文档。

> **版本说明：** OpenWeather 的免费额度、可用产品、订阅档位和调用限制会随官方策略调整；本章只说明 API Key 获取和本地配置流程，具体额度与计费以当前 OpenWeather Pricing 页面和账号订阅页为准。

![OpenWeather 用户后台「API keys」页：查看已有密钥状态，或通过 Create key 生成新密钥（教程中用于配置天气工具）](https://didilili.github.io/ai-agents-from-zero/images/17/17-5-1-1.png)
...
对应教程章节：第 9 章 - Tools 工具调用 → 5、天气助手实战：把 Tool 跑成业务闭环 → 5.2 定义天气查询工具
...
![Function calling 技术思路：普通对话（用户 ↔ 大模型）与「查询天气」场景下，大模型发出函数调用请求 → 外部函数 get_weather → 请求 OpenWeather API → 响应回到模型再生成用户可见回复](https://didilili.github.io/ai-agents-from-zero/images/17/17-5-3-1.jpeg)
...
对应教程章节：第 9 章 - Tools 工具调用 → 5、天气助手实战：把 Tool 跑成业务闭环 → 5.4 案例：天气助手完整链路 / 5.5 课程案例写法与官方主线的关系
...
- 和 第 8 章记忆与对话历史 的关系：工具调用结果经常也要进入消息流，与对话历史一起参与后续推理
- 和 第 5 章Agent智能体 的关系：Agent 会在 Tool 基础上进一步解决“什么时候调、调几次、按什么顺序调”
- 和 第 12 章MCP模型上下文协议 的关系：MCP 解决的是“如何用标准协议把外部能力开放给模型”，可以看作更通用、更标准化的工具接入方式
...
**建议下一步：** 先把本章 5 个案例全部跑一遍，重点观察“工具描述、参数 schema、模型返回 tool_calls、程序执行工具”这四个环节；然后继续学习 第 5 章 Agent 智能体，你会更容易理解为什么 Agent 的核心不是“多了几个 API”，而是在 Tool 基础上增加了自主决策与多步编排能力。
